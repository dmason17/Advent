...
for x in splitInput:
    if decompress:
        if count < int(numSplit[0]):
            toRepeat = toRepeat + x
            count = count + 1
        else:
            for y in range(0, int(numSplit[1])):
                final = final + toRepeat
            if x == '(':
                marker = True
            else:
                final = final + x
...
print len(final.strip())



def decompressString(charList):
    length = 0
    marker = False
    decompress = False
    num = ''
    ...
    for x in charList:
        if decompress:
            if count < int(numSplit[0]):
                ...
                length = length + (decompressString(list(toRepeat)) * int(numSplit[1]))
                if x == '(':
                    marker = True
                else:
                    length = length + 1
...
print decompressString(splitInput)
